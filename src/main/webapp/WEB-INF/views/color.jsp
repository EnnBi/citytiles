...
<div class="col-12 grid-margin">
	<div class="card">
		<div class="card-body">
			...
			<c:choose>
				<c:when test="${edit}">
					<c:set value="${pageContext.request.contextPath}/color/update" var="action" />
					<c:set var="caption" value="Update" />
				</c:when>
				<c:otherwise>
					<c:set value="${pageContext.request.contextPath}/color/save" var="action" />
					<c:set var="caption" value="Save" />

				</c:otherwise>
			</c:choose>

			<form:form action="${action}" modelAttribute="color" method="post">
				<form:hidden path="id" />
				<div class="row">
					<div class="col-md-12">
		...
		<div class="card-body">
			...
			<div class="table-responsive">
				<table class="table table-striped">
					...
					<c:url var='updatelink' value="/color/edit" />

					<c:url var="deletelink" value="/color/delete" />
					<tbody>
						<c:forEach items="${colors}" var="templist">
							<tr>
								<td>${templist.name}</td>
