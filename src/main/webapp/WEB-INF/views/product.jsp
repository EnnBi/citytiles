...
<div class="col-12 grid-margin">
	<div class="card">
		<div class="card-body">
			...
			<c:choose>
				<c:when test="${edit}">
					<c:set value="/product/update" var="action" />
					<c:set var="caption" value="Update" />
				</c:when>
				<c:otherwise>
					<c:set value="/product/save" var="action" />
					...
					<c:forEach items="${list}" var="templist">
						...
						<tbody>
							<tr>
								...
								<td>${templist.name}</td>
								<td><c:forEach items="${templist.sizes}" var="size"
								 >${size.name}   </c:forEach></td>
							...
							<ul class="pagination">
								<c:if test="${currentPage !=1}">
									<li class="page-item"><a class="page-link"
										href="/product/pageno=${currentPage - 1}">Previous</a></li>
								</c:if>
								...
								<c:if test="${currentPage!= totalPages}">
									<li class="page-item"><a class="page-link"
										href="/product/pageno=${currentPage + 1}">Next</a></li>
								</c:if>
							</ul>
						</c:if>
					</nav>
